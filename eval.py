...
def evaluate_with_grid_search(hyper_params, kernelized_rr_forward, data, item_propensity, train_x, topk = [ 10, 100 ], test_set_eval = False, VAL_METRIC = "HR@100"):
    best_metrics, best_lamda = None, None

    for lamda in [ float(1e-5), float(1e-3), 0.1, 1.0, 5.0, 50.0 ]:
        ...
        if (best_metrics is None) or (val_metrics[VAL_METRIC] > best_metrics[VAL_METRIC]): best_metrics, best_lamda = val_metrics, lamda

    if test_set_eval:
        hyper_params['lamda'] = best_lamda
        return evaluate(hyper_params, kernelized_rr_forward, data, item_propensity, train_x, test_set_eval=True, topk=topk), best_lamda
    else:
        return best_metrics, best_lamda
